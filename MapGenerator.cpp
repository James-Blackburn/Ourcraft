...
void MapGenerator::generateMap(std::vector<std::unique_ptr<Chunk>>& chunks, std::unique_ptr<Player>& player, bool& generated, std::string& currentText)
{
    ...
    for (int x=0; x<MAP_X; x+=BLOCK_SIZE)
    {
        ...
        if (blockHeights[(blockHeightIndex + 1) % blockHeights.size()] > waterLevel + BLOCK_SIZE
            || blockHeights[(blockHeightIndex - 1) % blockHeights.size()] > waterLevel + BLOCK_SIZE)
            nextToWater = true;
    ...
    allocateChunks(chunks, tiles, blockHeights);
    currentText = "Spawning Player";
    spawnPlayer(blockHeights, player, waterLevel);
    generated = true;
}

void MapGenerator::allocateChunks(std::vector<std::unique_ptr<Chunk>>& chunks, std::vector<std::shared_ptr<Tile>>& tiles, std::vector<int>& blockHeights)
{
    int y = 0;
