...
void PlayingState::fillVertexArray(sf::VertexArray& vertices, std::vector<Tile*>& renderTiles)
{
    for (int i=0; i<renderTiles.size(); i++)
    {
        ...
        // Get quad colours
		const int quad0Colour = tile->getQuadBrightness(player.get(), 0);
		const int quad1Colour = tile->getQuadBrightness(player.get(), 1);
		const int quad2Colour = tile->getQuadBrightness(player.get(), 2);
		const int quad3Colour = tile->getQuadBrightness(player.get(), 3);
